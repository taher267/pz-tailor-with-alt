requirejs.config({
    paths: {
        "jquery": "jquery",
        "assert": "assert",
        "print-in-page": "jquery.print-in-page"
    }
});
...
require(["jquery", "assert", "print-in-page"], function ($, assert, pip) {
    ...
    */
    ...
    $('.click-to-print').click(function (e) {
        $('.print-date-time').removeClass('d-none');
        $('.print-date-time').css('display', 'flex');
        $('.measure_item_name').addClass('d-none');
        setTimeout(() => {
            $('.print-date-time').addClass('d-none');
            $('.measure_item_name').removeClass('d-none');
        }, 0);
        var btn = $(this);
        ...
        for (var i = 0, len = clsarr.length; i < len; i++) {
            ...
            if (match) {
                zone.push(".measurementPrint-" + match[1]);
            }
        }
        zone = zone.join(", ");
        var $el_set = $(zone);
